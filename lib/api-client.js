...
class ApiClient {
    ...
    get(url, page, limit, config) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (limit && (limit === common_1.Limit.ALL || limit > maxLimit)) {
                ...
                return yield this.callWithRetries("get", (0, util_1.appendQueryString)(url, {
                    page,
                    limit: limit === common_1.Limit.DEFAULT
            ...
            while (true) {
                try {
                    ...
                    if (!this.config.failOn404 &&
                        ...
                        const response = error.response;
                        if (request && response) {
                            throw new Error(`Failed to call BigCommerce API ${request.method} ${request.protocol}://${request.host}${request.path}. Response Status: ${response.status}, Response: ${JSON.stringify(response.data, null, 0)}. Retries: ${retries}. Error: ${error.message}.`, { cause: error });
                        }
                        if (request) {
                            if (error.code === "ECONNABORTED") {
                                throw new Error(`Failed to call BigCommerce API ${request.method} ${request.protocol}://${request.host}${request.path}. Timeout after ${this.config.timeout} ms. Retries: ${retries}. Error: ${error.message}.`, { cause: error });
                            }
                            throw new Error(`Failed to call BigCommerce API ${request.method} ${request.protocol}://${request.host}${request.path}. Retries: ${retries}. Error: ${error.message}.`, { cause: error });
                        }
